...
citojs只是对双端比较做了一个简单的运用，算是双端比较+map定位，基本思路如下：
```

第一步：双端比较算法，简单的过滤掉一些只需要一个替换操作就可以完成的节点
...
又发现A.end == B.start，也就是f的位置正好对应上，那么此时我们只需要将旧列表的f元素插入到A.start处就即可
A: -> [b c d e] <-
B: -> [d h e c] <-
...
此时已经没有可以用第一步方法直接处理的数据了，使用一个额外的object来记录每个key对应的旧元素，没有key的可以统一理解成是一个null值
A: -> [b c d e] <-
B: -> [d h e c] <-
M: {b c d e}

从新列表最后往前遍历（从前往后遍历也是可以的，这里并不是技术点）
A: -> [b c d e] <-
B: -> [d h e c] <-
             ^
nodeList: [a b f c d e g]
...
发现c在M中存在，则去做替换操作，而M中已经用过的标记为null
A: -> [b c d e] <-
B: -> [d h e c] <-
           ^
...
发现e在M中存在，则去做替换操作，其实此时e的位置已经是正确的位置了，不过还是做了替换操作
A: -> [b c d e] <-
B: -> [d h e c] <-
         ^
nodeList: [a b f d e c g]
...
此时发现h在M列表中不存在，则进行插入操作
A: -> [b c d e] <-
B: -> [d h e c] <-
       ^
nodeList: [a b f d h e c g]
M: {b null d null}

此时发现d在M列表中存在，则去做替换操作。
由于实际上此时的位置已经是其调整后的位置了，不过还是做了替换操作，可能浏览器侧会对此做了优化
A: -> [b c d e] <-
B: -> [d h e c] <-
nodeList: [a b f d h e c g]
M: {b null null null}

此时发现b还在M表示该节点是多余的，需要进行删除操作
nodeList: [a b f d h e c g]
             ^(remove)

最后citojs还会遍历旧列表找出没有处理过的元素，然后做删除操作

至此结束，最终的nodeList与新列表一致
```

这里需要说明的是cito.js早已被作者停止维护
...
2. 针对有key的列表节点，则是先使用双端比较算法过滤掉一部分可以使用一个插入操作就可以将他们的位置从旧列表调整新列表的key节点，然后使用最长上升子序列（用他们在新列表的下标位置）来确认不需要调整的节点，接着将需要调整的节点移动到对应的上升子序列中对应的位置
...
我们用一个新的数组，每个位置都初始化为-1，然后一一记录旧列表在新列表中的下标位置
A: -> [b c d e f] <-
B: -> [c b h f e] <-
P: [-1 -1 -1 -1 -1]

b在新列表1的位置，c在新列表0的位置，d在新列表中没有，不处理，e在新列表4的位置，f在新列表中3的位置
...
至此我们得到一个序列，从这个序列中找到最长上升子序列，然后将不在这个子序列的元素依次调整到对应的位置即可，这里不详解最长上升子序列。

P: [1 0 -1 4 3]中最长上升子序列可以是[1 4]也可以是[0 4]或者是[1 3]。
这个就是看代码中如何去实现最长上升子序列了，不过最长上升子序列一个最基本的原则是。
当处理到当前位置时，要么增加一个长度要么将之前的最大值降低。
```

## snabbdom.js

vue2.0的diff算法借鉴于snabbdom，本质上两者从算法角度看是没有差异的，只是说对于DOM节点差异性变更有些许不同，snabbdom像是加强版双端比较+map快速定位。

整体思路很简单，跟kivi很类似，唯一不同的是，他将双端比较放到每一次循环里面，而不是跟kivi那样，一个列表只会调用一次双端比较，snabbdom每个节点都会调用一次双端比较，如果不一致才会调用与citojs类似的map快速定位。

```
初始状态下新旧列表以及此时的node列表情况：
A: -> [a h c d e f g] <-
B: -> [a b f d c g] <-
nodeList: [a h c d e f g]
...
此时发现b元素在旧列表中没有则进行添加操作
A: -> [h c d e f] <-
B: -> [f d c] <-
nodeList: [a b h c d e f g]
             ^(insert)

此时发现新列表的f和旧列表的f满足交换原则
A: -> [h c d e] <-
B: -> [d c] <-
nodeList: [a b f h c d e g]
               ^

此时发现d在旧列表中存在，但是并不在头尾，头尾指针此时不能变化，需要标记为null
A: -> [h c null e] <-
B: -> [c] <-
nodeList: [a b f d h c e g]
                 ^

同理发现c也一样
A: -> [h null null e] <-
B: -> [] <-
nodeList: [a b f d c h e g]
                   ^

发现旧列表还有剩余，遍历A列表，发现不为null则执行删除操作
A: -> [] <-
B: -> [] <-
nodeList: [a b f d c g]
...
diff逻辑与snabbdom并无太大差异

上述对历史上对虚拟DOM的diff算法有过重大影响的算法都已经将他们的diff的核心算法给抽象了出来，也许很多人会疑惑，上面介绍的内容并不全面，是的，个人将很多比较单节点的逻辑都给忽略掉了。或者说很多情况单单比较key并不能判断两个元素一致，这就涉及到了大家在比较的时候使用的函数了，有些开源框架中会将key相同但是组件元素已然发生变化的节点当作一个全新的节点，然而这也只是改变了判断两个key直接是否一致的实现逻辑，核心部分不会脱离上述所讲的内容